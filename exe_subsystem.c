...
extern void draw_window_pure(int x, int y, int width, int height,
                             uint32_t border_color);

/* OOBE aşamalarını takip eden global değişkenin tanımı (Header'daki extern'i
 * besler) */
int setup_stage = 0;

/* Özel pencereler ve arayüz bileşenleri için gövde doldurma fonksiyonu */
void draw_custom_window(int x, int y, int width, int height, const char *title,
                        uint32_t body_color) {
  // Derleyicinin title uyarısını susturmak ve ileride kullanmak üzere
  // sabitlemek
  (void)title;
